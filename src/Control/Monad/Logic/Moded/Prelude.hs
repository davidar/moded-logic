...
import Prelude (Eq(..), Fractional(..), Num(..), (.))
...
sum =
  R2
    {callIO = pure . Prelude.sum, callII = \xs s -> guard (Prelude.sum xs == s)}

maximum ::
     (Alternative m, Prelude.Foldable t, Prelude.Ord a) => Relation2 m (t a) a
maximum =
  R2
...
print = R1 {callI = liftIO . Prelude.print}

show ::
     (Applicative m, Prelude.Show a, Prelude.Read a)
  => Relation2 m a Prelude.String
show = R2 {callIO = pure . Prelude.show, callOI = pure . Prelude.read}

observeAll :: Applicative m => Relation2 m (Relation1 Logic.Logic a) [a]
observeAll = R2 {callIO = pure . Logic.observeAll . callO}
