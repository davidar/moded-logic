{-# LANGUAGE DeriveFunctor, DeriveFoldable, FlexibleContexts,
  FlexibleInstances, OverloadedStrings, QuasiQuotes #-}
...
cGoal procs p r
  | Atom {} <- extract p (body r) = cAtom procs p r
cGoal procs p r =
  Set.unions [cComp procs p' r | p' <- extendPath p r] `Set.union`
